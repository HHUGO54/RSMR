...
function Modal({
  ...
  const inputClass =
    "w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500";
      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl flex flex-col">
        ...
        <div className="flex justify-end gap-2 px-5 pb-5">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-xl bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-60 transition-colors"
          >
...
export default function AspirantesClient({ aspirantes, isAdmin }: Props) {
  ...
  async function handleSaveNew(data: FormState) {
    const res = await fetch("/api/aspirantes", {
      ...
      body: JSON.stringify({ nombre: data.variable, ...data }),
    });
  ...
  async function handleSaveEdit(id: number, data: FormState) {
    const res = await fetch(`/api/aspirantes/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nombre: data.variable, ...data }),
    });
  ...
  async function handleDelete(id: number) {
    setDeleting(id);
    try {
      await fetch(`/api/aspirantes/${id}`, { method: "DELETE" });
      router.refresh();
    } finally {
      setDeleting(null);
    }
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      {modal?.mode === "edit" && (
        <Modal
          ...
          initial={{
            variable: modal.item.variable ?? "",
            notas: modal.item.notas ?? "",
          }}
          onClose={() => setModal(null)}
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">Aspirantes</h1>
      ...
      {aspirantes.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {aspirantes.map((a) => (
            <div
              key={a.id}
              className="bg-white rounded-2xl border border-gray-200 p-4 flex flex-col gap-3 hover:shadow-sm transition-shadow"
            >
              ...
              {a.notas && (
                <p className="text-xs text-gray-400 italic leading-relaxed">{a.notas}</p>
              )}
            </div>
          ))}
        </div>

...
export default function VariableDetalleClient({ variable, activos: inicial, variables }: Props) {
  ...
  // Activos que siguen en esta variable
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <div className="flex items-center gap-3">
        <Link
          href="/rayado/grafica"
          className="rounded-lg border border-gray-200 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
        >
          <ArrowLeft size={18} />
        </Link>
        <div>
          <p className="text-xs text-gray-400 uppercase tracking-wide font-medium">Gráfica</p>
          <h1 className="text-2xl font-bold text-gray-900">{variable}</h1>
          <p className="text-sm text-gray-500 mt-0.5">{activos.length} miembros — toca un nombre para cambiar su variable</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden divide-y divide-gray-50">
        {activos.length === 0 && (
          <p className="text-center py-10 text-gray-400 text-sm">Sin miembros en esta variable</p>
        )}
        {activos.map((a, i) => {
          ...
          return (
            <div key={a.id} className={`transition-colors ${cargando ? "opacity-50" : ""}`}>
              ...
              >
                ...
                {a.seccion && (
                  <span className="shrink-0 inline-flex items-center rounded-md bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
                    §{a.seccion}
                  </span>
                )}
              ...
              {/* Panel de variables para cambiar */}
              {abierto && (

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl flex flex-col">
        ...
        <div className="flex justify-end gap-2 px-5 pb-5">
          <button onClick={onClose} className="rounded-xl border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
            Cancelar
          </button>
...
export default function MovilizadoresClient({ movilizadores, isAdmin }: Props) {
  const router = useRouter();
  const [modal, setModal] = useState<{ mode: "new" } | { mode: "edit"; item: Movilizador } | null>(null);
  const [deleting, setDeleting] = useState<number | null>(null);

  function refresh() { setModal(null); router.refresh(); }

  async function handleSaveNew(data: FormState) {
    const res = await fetch("/api/movilizadores", {
      method: "POST",
  ...
  async function handleSaveEdit(id: number, data: FormState) {
    const res = await fetch(`/api/movilizadores/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
  ...
  async function handleDelete(id: number) {
    setDeleting(id);
    try {
      await fetch(`/api/movilizadores/${id}`, { method: "DELETE" });
      router.refresh();
    } finally {
      setDeleting(null);
    }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      {modal?.mode === "new" && (
        <Modal title="Nuevo Movilizador" initial={EMPTY} onClose={() => setModal(null)} onSave={handleSaveNew} />
      )}
      {modal?.mode === "edit" && (
        <Modal
          title="Editar Movilizador"
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900">Movilizadores</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {movilizadores.length} {movilizadores.length === 1 ? "registro" : "registros"}
          </p>
        </div>
        {isAdmin && (
          <button
            ...
            <div key={m.id} className="bg-white rounded-2xl border border-gray-200 p-4 flex flex-col gap-3 hover:shadow-sm transition-shadow">
              <div className="flex items-start justify-between gap-2">
                <div className="rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 flex-1">
                  ...
                  <p className="text-sm font-semibold text-blue-800">{m.variable ?? m.nombre}</p>
                </div>
                {isAdmin && (

...
interface Activo { id: number; nombre: string; movilizacionAsignada: string | null; }
...
export default function MovGraficaClient({ datos, sinAsignar, totalActivos, activos }: Props) {
    ...
    return (
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center text-gray-400">
          ...
          <p className="text-sm mt-1">Asigna movilizadores en <span className="text-blue-600 font-medium">Definir</span> para ver la gráfica</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
        ...
        {busqueda.trim() && (() => {
          ...
          if (resultados.length === 0) {
            return <p className="text-sm text-gray-400 px-1">Sin resultados</p>;
          }
          return (
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        <p className="text-sm font-semibold text-gray-700 mb-2">Miembros por movilizador</p>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={datos} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
            ...
            <XAxis dataKey="variable" tick={{ fontSize: 12, fill: "#6b7280" }} axisLine={false} tickLine={false} />
            <YAxis allowDecimals={false} tick={{ fontSize: 12, fill: "#6b7280" }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ borderRadius: 12, border: "1px solid #e5e7eb", fontSize: 13 }} cursor={{ fill: "#f0f9ff" }} />
            <Bar
              dataKey="total"
              radius={[6, 6, 0, 0]}
              maxBarSize={64}
              ...
              onClick={(data: any) => router.push(`/movilizacion/grafica/${encodeURIComponent(data.variable)}`)}
            >
